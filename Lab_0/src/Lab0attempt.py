...
pinC0 = pyb.Pin(pyb.Pin.board.PA5, pyb.Pin.OUT_PP)# (Pin.OUT_PP) configures the pin for output, with push-oull control
pinB0 = pyb.Pin(pyb.Pin.board.PA3, pyb.Pin.OUT_PP)
...
def timer_int(tim_num):
    ...
    Queue.put(data)# Stores data in queue 

def step_response(inputt):
    """!
